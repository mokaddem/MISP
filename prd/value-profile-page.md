...
(`live/25-timeline.md` §25). **Analyst data — the Collaboration tab —
closed the same day**, and it grew a third panel doing it: the coverage
survey's narrative report list, which had been owed to this tab since
§4.5 and had no endpoint anywhere (`live/26-analyst.md` §9.2). Four
readings of the built tab followed the build and are §17–§20 there — the
rename, a lane bug, links on every chip that names a record, the report
badge's audience, and the Overview's preview card — and §21 re-ran the
verification against all four rather than assuming §12's pass still
held.

**Phase 27 built History the same day**, which is the last unblocked
tab and the sixth live phase. It reads the database through the audit
reader phase 25 wrote for it, and `live/27-history.md` §3 is where three
of the tab's own design assumptions turned out to be false on real data
— none of them in a direction that breaks the panel, all three in the
direction that makes it quieter than the fixture promised.

**It also fixed a disclosure the Timeline had been shipping.**
`AuditLogsController::eventIndex` removes a foreign-org actor's email
from every row before rendering it to a non-site-admin, and it does that
in the controller rather than in the conditions — so phase 25's reader,
which inherits that model's conditions, inherited none of the redaction
and named the actor on every row. `26-analyst.md`'s pattern again: the
subset claim was about which rows, not about what each row carries.
...
panel-level record — **all thirty-three endpoints read live data, and
thirty-one of them carry their numbers.** The two without
them are `viewRelationReferences` and `viewRelationExternal`, built by
phase 24 and never recorded. `viewAnalystPreview` was a third until
phase 26 §20 converted it and [`live/22b`](value-profile-live/22b-standalone-proposals.md)
§6.5 measured it. Add the
two Relationships rows whose `Q` predates the scan B15 rebuilt them onto
and that is the four imperfect rows named there and in §20.4.

**The thirty-third arrived after the campaign closed.**
[`live/31-overview-balance.md`](value-profile-live/31-overview-balance.md)
gave the Overview a Reporting card — who reports the value, and in which
months — and cut its occurrence preview from twenty-five rows to eight
to pay for it. It is the first endpoint on this page added for a
**layout** reason: the occurrence card measured 792px of a 1531px pane
and was answering, badly and from a capped sample, two questions the
Assessment and Timeline tabs each answer properly in one aggregate.
Nothing new is computed — both halves call the method the owning tab
calls — so the page gained a summary without gaining a source of truth.

**The one card phase 26 left knowingly wrong was paid for the same
day.** The Overview's analyst preview read the fixture beside three live
Collaboration panels — phase 26's second open call, first answered
*leave it and record it* on the rule that a tab's row belongs to its own
phase (`live/26-analyst.md` §11). The maintainer reversed that after
§19: `forAnalystPreview` now runs the tab's own `analystContext` and
nothing else, so the card's counts and the tab's counts are one array
and cannot drift. §20 is the conversion, and §20.2 the three things that
fell out of it — newest-first across both kinds, proposals counted in
the empty state, and the organisation chip made openable.
...
out of order; `value_external` went live with phase 24's fourth
...
[`analyst-profile/`](analyst-profile/), fifteen documents, fifteen decisions,
...
one-number "MALICIOUS 84" claimed more than an engine reading MISP tables can
honestly measure, so the tab becomes an **Assessment** on three axes: lean
(what the record asserts), relevance (whether it still holds) and quality (how
well evidenced it is, and the only axis the exact-sum invariant now governs).
**D12 makes the signal catalogue a directory** — a signal is a PHP file an
instance admin drops in `app/Lib/ValueSignals/`, discovered the way
`Workflow` finds its custom modules, so nothing in code holds a list of
...
| 31 | The Overview stops being an occurrence table — the preview shortened, and a Reporting card in the space | `live/31-overview-balance.md` | **built 2026-09-14** — §1 the measurement that started it, §2 its seven decisions, §4 the cost, §5 the three candidates that were weighed and not built, §6.1 the two defects the build found |
| 32 | The page could only see a seventh of the labelling — event tags beside attribute tags, on four surfaces | `live/32-tag-scope.md` | **built 2026-09-14** — §1 the measurement, §2 why no new access rule is needed, §3 the four surfaces, §4 the height it cost and what brought it back, §5 two performance findings |
| 33 | The two tag scopes fold into one list — the split, the glyph and the second facet withdrawn; clusters grouped by galaxy, and reaching the occurrences pane at last | `live/33-tag-scope-fold.md` | **built 2026-09-14** — §1 why the split was the wrong shape, §2 the `×N` it costs and why it is not bought back, §3 the four surfaces, §4 the three height budgets, §5 the Galaxies column and its facet, §7 `Q` down from 9–18 to 8–15, §8 what was checked live |
...
|---|---|---|---|
| **29** | **Overview** | [`live/29-overview.md`](value-profile-live/29-overview.md) | **CLOSED 2026-09-14 — the last live phase, and the fixture's last readers.** Converts the occurrence card, the context card, the two thirds of the Lifecycle card phase 5 did not take, and **the page frame** — the only synchronous read on this page, now one `forFrame` at six queries. Three fixture keys were **withdrawn** rather than converted: the pivot rail was five enrichment answers MISP does not store, the correlation count a per-occurrence query for a number phase 24 showed means nothing about a value, and the hidden-occurrence count a disclosure rather than a caveat. **§14 is the build log and its five findings are the phase's real content** — the occurrence card counted one page and headed it *19 events* beside a strip reading *20*; the context card rendered `443`'s 3,860 tags as a **2.9 MB** fragment, which is what `Value::topTagsFor` and a cap of 60 exist for; the ordinal scale sorted backwards and then drew *position 1* for every numerically-keyed taxonomy, because PHP casts numeric array keys to integers; and the fact strip still linked to `#tab-verdict`, which D11 renamed eleven days earlier. **§15 is a live probe under two readers' permissions** — 163 checks — and it found a sixth defect: the galaxy list was drawn from a cap it shared with the labels, so a CIRCL org admin seeing 45 of `443`'s 1,844 events was shown **more** clusters than a site admin seeing all of them. Not a leak; a list that was never the value's galaxies. **§16 took the hand-on §14.10 had just written down**: the sightings count `forTabCounts` refused two phases ago, because getting the viewer's number meant running `Sightings_policy` over fetched rows. Every branch of that policy turns out to be a statement about the *event*, so `Sighting::visibilityConditions` expresses it as a predicate and `Value::sightingCountsFor` counts in one indexed aggregate — **verified against `listSightings` under all four policies and three readers, 240 checks**. The tab badge is back after carrying a fixture literal reading 17 beside a panel reporting 53. **65 harness checks, 403 probe checks**, six values over HTTP, both themes. `ValueProfileFixture` has no reader in `ValuesController` |
...
| **31** | **Overview, again** — its balance rather than its data | [`live/31-overview-balance.md`](value-profile-live/31-overview-balance.md) | **BUILT 2026-09-14 — the first phase after the campaign closed, and the first about layout rather than data.** The Overview's occurrence card measured **792px of a 1531px pane**, 52% of the column it sat in, and clipped itself at 70vh so a *summary* scrolled inside a page already scrolling. Three causes, and only one was the row count: 25 rows, the shared event badge at **95px a row**, and the `70vh` clip — plus a six-action mass toolbar with every action disabled. **The cap is 8, the rows are one line, the toolbar and the selection column are gone**, and `Last seen` with them, because the fact strip already prints it and *when* is now the new card's whole left half. 433px, 37px a row, and **9 queries down to 5** — `attachCreatorOrgs`' one-select-per-organisation N+1 is bounded by the cap, so cutting the cap cut the queries. In the space, **`viewReporting`**: a bar a month over the value's whole life beside occurrences-per-organisation with each one's `to_ids` stance — the Timeline tab's *Activity on this value* and the occurrence half of the Assessment tab's *Who says what*, **each read through that tab's own method** so the three surfaces cannot disagree. Three queries flat, on `443`'s 48,255 occurrences as on `sage.png`'s one. **The rail was deliberately not touched**: it summed to 1466px against the left column's 1483, so it — not the occurrence card — sets this tab's height, and a fifth rail card would answer the complaint by making the page taller. That is why the free *What would change this* card is in §5 rather than built. **Two defects the build found**: the organisation slice was non-deterministic where counts tie, so *2 further organisations not shown* named a different two each load; and the type chip's empty state claimed *no occurrence has type X* from a sample of eight — false on `8.8.8.8`, whose five `ip-src` rows are all older than its eight newest |
| **22** | **Occurrences** | [`live/22-occurrences.md`](value-profile-live/22-occurrences.md) | **built** — capped at 300 rows (§6); review added the resolved distribution chain and two working time ranges (§13), sortable columns and a reader-chosen page size (§14), a brush on each time range (§15), and an Object facet (§16). **Standalone proposal rows are built** (2026-09-14) as a `Proposed additions` panel under the table, outside its counts: [`live/22b-standalone-proposals.md`](value-profile-live/22b-standalone-proposals.md). **Still owes the feed column** — [`value-profile-coverage.md`](value-profile-coverage.md) §5.1 |
...
- A verdict scoring engine. The page displays a verdict; it does not compute one.
  ...
  **Designed 2026-09-02/03 and still unbuilt** — the engine, its configuration
  object and the three-axis Assessment that replaces the one-number verdict are
  [`analyst-profile/`](analyst-profile/), ten phases of which seven are built as of 2026-09-07 — what is left before the tab can read them is phase 9.
  The scope note this section used to point at,
  [`value-profile-verdict-engine.md`](value-profile-verdict-engine.md), is
  superseded by it.
