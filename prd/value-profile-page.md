...
can be opened. Blocked on phase 20, and it inherited one decision phase 19 made
awkward — the drag gesture was already spent on selecting a period, so zoom got
its own: four buttons on the visible span, with the bucket unit following from
it. The measurement §13.1 asked for came back saying bars are cheap enough to
ship at every grain, so no zoom level fetches. History and Sightings converted;
the Timeline declines, because twelve monthly bins is not a span that needs
opening.
