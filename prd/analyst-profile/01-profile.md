...
| D22 | **The shipped default declares a CIRCL-first mapping.** `default-v1.json` v9 fills `auto_run` for thirteen types and twelve modules, nine of them CIRCL-operated; v10 carries the same mapping with the description corrected. It runs nothing and enables nothing; it **narrows** — a type it names arrives with those modules ticked and the rest unticked | `09b-revisions.md` 3.22 |
