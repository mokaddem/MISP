...
**The third axis followed it the same day** (§18 there). Lean looked finished
because the flagship value takes the one exit that carries prose, and
`ValueLeanTool::leanFor()` has seven: the other six said nothing, so an
ordinary value asserted a threat without naming the organisations that asserted
it or the threshold they cleared. `stances` and `rule_errors` were both
computed per value and read by no template in the application. The lean band
draws the two counts, the split against both supermajority marks, a sentence
per exit and any conflict rule that could not run — for no query, again. The
exit is **named by the engine rather than re-derived**, because rules 3 and 5
both answer `benign` and only the order they are written in tells them apart.
What it reaches that nothing else did is rule 6: a value drawn as contested
...
none, and a reader whose `ui_theme` is `Default` has one. Every analyst
not on `Overmind` had been getting a 500 from the Value Profile page
since phase 7 shipped it ([`09c-wiring.md`](09c-wiring.md) §7.1).
...
[`12-assessment.md`](12-assessment.md). D12 to D15 were taken as
phases 2 to 7 were built, and **D16 to D22 during phase 8's two
revision rounds, 2026-09-10 to 2026-09-12** — the round documents
own the reasoning, the rows below say what was decided. This is the
index, and it is complete: a decision that is not here is not a
decision this feature has taken.
