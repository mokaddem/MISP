...
*Confirmed 2026-09-03, against the sibling checkout `~/git/misp-warninglists`:
0 of 89 `list.json` files carry a `category` field. The same pass found the
gap runs one layer deeper — `Warninglist::__updateList()` does not import the
field either (`Warninglist.php:394`), so an upstream fix alone would change
nothing. The staged resolution — a hardcoded map in `app/Lib/Tools/` for V1,
the upstream PR plus the core import fix as V2 — is `07-reference.md` §3.3.*
...
*Addendum 2026-09-03 (review C-series): the name sits near MISP 2.5's
existing **Analyst Data** family — `perm_analyst_data`, the
notes/opinions/relationships objects, and `analyst_data_blocklists`, the new
table's alphabetical neighbour — a collision this section did not weigh. It
was weighed later and the name stands: an Analyst Profile configures how the
analyst's assessment is computed; Analyst Data is what analysts write about
records; the two never share a screen or a permission. Recorded so a reviewer
asking "is this part of Analyst Data?" finds the answer — no, and
deliberately.*
...
|---|---|---|---|---|
| C1 | **Signal catalogue** — which signals are evaluated at all | the ledger's rows | 24 hand-written rows across 4 values | Q2, ~~Q11~~ D12 — a directory listing |
...
| C5 | **Escalation / conflict rules** — named overrides | CONFLICTED, and `changers` | one named rule, prose only (§3.2c) | Q2, ~~Q11~~ D12 — a directory listing |
...
Ordered by how much else depends on the answer. **Q1–Q4, Q6, Q8, Q11 and Q12
are decided** and the reasoning is carried below; the main PRD
([`01-profile.md`](01-profile.md)) consolidates them. Q5, Q7, Q9 and Q10
...
**~~Q11 — Is there an extension point in v1?~~ Decided 2026-09-07 — yes, as
D12.** *"They should be able to drop a php file in a directory and it should be
picked up by the system, so nothing hardcoded."* Signals and escalations — C1
and C5 — resolve to classes **discovered from the filesystem**:
`app/Model/ValueSignals/` for the shipped catalogue and
`app/Lib/ValueSignals/` for what an instance admin adds, mirroring
`Workflow`'s two module roots. No list of signals exists anywhere in code, so
C1 and C5 accommodate an addition without a format change or a release.

The expression-language half of the question is answered the other way: a
signal is a class, not data. And the "visible to others" half dissolves — a
file on the filesystem is instance-wide by construction, so the only
per-profile question is whether an id is *enabled*, which D3's resolution
already settles. `03-signals.md` §8 is the mechanism and its five rules; §8.7
records what this costs — writing a signal needs filesystem access, so it is
an instance-admin capability and an analyst asks for one.
