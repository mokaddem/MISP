...
| Overview | `viewContext` | `value_context` | **9–18** | the *taxonomies present*, not the value's size — four capped reads now, two scopes of labels and clusters | 2, two aggregates at 2 | **29**; `Q` and the second scope are [**32**](32-tag-scope.md)'s, whose §5.3 shows the time is `topTagsFor`'s and predates it; §14.4 there is the 2.9 MB fragment that made it a cap and §15.1 the reader who saw more clusters for seeing less of the value |
