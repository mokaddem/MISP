...
| — | Verdict — the tab's three panels and the Overview's verdict card. **Renamed Assessment by D11** | [`../analyst-profile/10-wiring.md`](../analyst-profile/10-wiring.md) | **designed, blocked on an implementation — which has started.** It is the Analyst Profile corpus's **phase 9** and needs that corpus's phases 1–5: the store (**built 2026-09-07**), the signal engine and its filesystem loader, the lean and bands, exclusions, and relevance. `../analyst-profile/01-profile.md` §1.4 is the phase table |
