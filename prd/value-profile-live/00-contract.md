...
| 29 | Overview — three panels and the page frame: the occurrence card, the context card, the two thirds of the Lifecycle card phase 5 did not take, and the only synchronous read on this page | [`29-overview.md`](29-overview.md) | **drafted 2026-09-13, not built** — §1 is the task board, §1.1 its eight decisions, three of which withdraw a fixture key rather than convert it, §8 the two things that cannot be converted at all, and §13 what each surface is fed and by which read. **It is the last live phase**: closing it leaves `ValueProfileFixture` with no reader in `ValuesController` and §14.8's unit-test double is all it is |
