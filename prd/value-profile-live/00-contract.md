...
Thirty-three endpoints, each rendering one element — twenty-seven until 2026-09-02, when three Relationships rows turned out to be missing rather than absent, thirty until 2026-09-05, when phase 26 added the Analyst tab's report list, thirty-one until 2026-09-06, when phase 28 split the Enrichment tab in two (a catalogue that runs nothing and a run that is the page's only POST), and thirty-two until 2026-09-14, when [phase 31](31-overview-balance.md) gave the Overview a Reporting card — **the first endpoint added after the campaign closed, and the first added for a layout reason rather than a data one**. This is the fine-grained
...
| Overview | `viewOccurrences` | `value_occurrences` | **6–7** | nothing — capped at **8** rows, and the totals are aggregates; the event-tag attach is one fetch for every event on the page | 1, two aggregates at 2 | **29**. `viewOccurrenceTable` took the same correction (§14.2, §15.2): both read one `occurrenceSummaryFor` rather than tallying their own capped rows. `Q` and the cap are [**31**](31-overview-balance.md)'s: it was 9 at a cap of 25, and the drop is the `attachCreatorOrgs` N+1 — one select per distinct organisation *among the rows fetched* — paying less because there are fewer rows |
| Overview | `viewReporting` | `value_reporting` | **3** | nothing — two grouped aggregates and one organisation lookup; 3 on a value with 48,255 occurrences | 1, two aggregates at 2 | [**31**](31-overview-balance.md). Who reports the value and when, read through the Assessment tab's `orgStanceFor` and the Timeline tab's `activityMonthsFor` so the three surfaces cannot disagree |
| Overview | `viewContext` | `value_context` | **8–15** | the *taxonomies present*, not the value's size — four capped reads, two scopes of labels and clusters folded into one list | 2, two aggregates at 2 | **29**; the second scope is [**32**](32-tag-scope.md)'s, whose §5.3 shows the time is `topTagsFor`'s and predates it; `Q` is [**33**](33-tag-scope-fold.md)'s §6, one `fetchGalaxyClusters` fewer once the two scopes fold; §14.4 of 32 is the 2.9 MB fragment that made it a cap and §15.1 the reader who saw more clusters for seeing less of the value |
| Overview | `viewAnalystPreview` | `value_analyst_preview` | **3–27** | organisations and anchors, not occurrences | 1, one aggregate at 2 | **26** §20; `Q` taken by [**22b**](22b-standalone-proposals.md) §6 when the event-report count was added — 27 on `8.8.8.8`, of which the count is one. [**35**](35-overview-borrowed.md) put the Collaboration tab's tug-bar on this card and the row **did not move**: the ledger it draws was already built by `analystContext` and deleted on the way out, and §2 there measures Q identically with and without |
...
| Occurrences | `viewOccurrenceTable` | `value_occurrence_table` | **8–10** | nothing — flat in occurrence count; the cluster attach is one `fetchGalaxyClusters` over the page's distinct galaxy tags, bounded by `OCCURRENCE_CAP` | 1, two aggregates at 2 | **22**; the tenth is the standalone-proposal fetch added by [**22b**](22b-standalone-proposals.md), which is §14.12's amended row taken as that section instructs. The cluster read is [**33**](33-tag-scope-fold.md) §5 — 26 distinct tags on the widest value, 11ms |
...
| 29 | Overview — three panels and the page frame: the occurrence card, the context card, the two thirds of the Lifecycle card phase 5 did not take, and the only synchronous read on this page | [`29-overview.md`](29-overview.md) | **CLOSED 2026-09-14 — T1–T16 done.** §1 is the board, §1.1 its eight decisions, three of which withdraw a fixture key rather than convert it, **§14 is the build log**: five defects the instance found and reading would not have — a card counting one page and labelling it a total, a context fragment of **2.9 MB** on `443`, an ordinal scale sorted backwards, every numerically-keyed taxonomy drawing *position 1*, and a fact cell still linking to the tab id D11 renamed. **§15 is the live probe**, 163 checks under two readers' permissions, which found a sixth: the galaxy list was drawn from a cap shared with the labels, so a reader who could see less of `443` was shown more of its clusters. **`ValueProfileFixture` now has no reader in `ValuesController`** and §14.8's unit-test double is all it is |
| — | Verdict — the tab's three panels and the Overview's verdict card. **Renamed Assessment by D11** | [`../analyst-profile/10-wiring.md`](../analyst-profile/10-wiring.md) | **CLOSED 2026-09-13.** The Analyst Profile corpus's **phase 9**, which needed that corpus's phases 1–5 and got all of them: the store, the signal engine and its filesystem loader, the lean and bands, exclusions, and relevance. All four rows are live and carry their numbers, the hero composes **D11's three axes** in one sentence, and the contested layout draws its two opposed cases. `../analyst-profile/01-profile.md` §1.4 is the phase table and `../analyst-profile/review-2026-09-13.md` the read-back over the built tab |
