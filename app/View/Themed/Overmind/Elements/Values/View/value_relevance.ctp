...
<div class="card shadow-sm mb-3 vp-panel vp-aside"
    ...
    <div class="p-3 d-flex flex-column gap-3">
        ...
        <?php else: ?>
            ...
            <?php
            /*
             * **The dates the axis is built on, side by side.**
             *
             * Three bugs in this feature were one column being read as
             * another: an encoding lag measured off a last-modified
             * timestamp (§7.11), an uncertainty flag that ignored the
             * sighting its own clock was running on (§7.12), and a
             * clock an edit could reset. All three were invisible from
             * the card, because the card showed conclusions and the
             * conclusions looked reasonable.
             *
             * So this is the provenance, at the size of a footnote: what
             * each date means, the column it comes from, and what this
             * value actually has. The row the clock used is marked,
             * which is the only one a reader has to find.
             *
             * Folded shut. It is reference material for the moment
             * somebody disbelieves the number above, not something to
             * read on the way past.
             */
            $facts = isset($valueProfile['timeline_facts'])
                ? $valueProfile['timeline_facts']
                : null;
            ?>
            <?php if ($facts !== null): ?>
                <?php
                ...
                $rows = array(
                    ...
                    array(
                        ...
                        'value' => $day($sightings['last_stamp'] ?? null),
                        'note' => empty($sightings['total'])
                            ? null
                            : sprintf(
                                __n(
                                    '%s sighting',
