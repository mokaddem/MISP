...
if ($extracted) {
     ...
     */
    $lead = sprintf(__n('%1$d line of text gave %2$d value.',
        '%1$d lines of text gave %2$d values.', count($rows)),
...
<?php   if ($unread > 0): ?>
    <p class="vi-unread">
        <b><?= h(sprintf(__n('%d line looks like a value this could'
            . ' not read.', '%d lines look like values this could not'
            . ' read.', $unread), $unread)) ?></b>
<?php     if ($report['unread_example'] !== null): ?>
        ...
        <?= h(__('extraction only finds what MISP can type, which is'
            . ' not everything MISP stores.')) ?>
<?php     else: ?>
        <?= h(__('Extraction only finds what MISP can type, which is'
            . ' not everything MISP stores.')) ?>
<?php     endif; ?>
...
<?php   endif; ?>
    <?= $this->element('Values/Index/reread', array(
        'extract' => 0,
        'label' => __('Use my lines instead'),
        'note' => $unread > 0
            ? __('To ask about every line exactly as you pasted it:')
            : __('If those were meant to be values, line by line:'),
    )) ?>
<?php elseif ($offerExtract): ?>
    <?= $this->element('Values/Index/reread', array(
        'extract' => 1,
        'label' => __('Find the values in it instead'),
        'note' => sprintf(__n('%d row is a piece of text rather than a'
            . ' value. If you pasted a report:',
            '%d rows are pieces of text rather than values. If you'
